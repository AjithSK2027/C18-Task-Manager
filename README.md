...
- Card-based responsive task feed (mobile-first, no horizontal overflow)
- Batch task creation: add multiple tasks in one form for same assignee
- Task notes + threaded comments
- Notification on task creation (email + WhatsApp)
- WhatsApp task notifications routed with department-specific head channel config
- Dedicated Team WhatsApp button for all users
- EOD report action available inside employee workspace (sends to company WhatsApp group)
- Manual and scheduled EOD report to WhatsApp group and email
- Premium visual theme: logo, high-end typography, refined palette, smooth animation
...
- Configure default and department-specific WhatsApp API keys/endpoints in `config`.
- Set `whatsappGroupInviteUrl` or `whatsappTeamUrl` for the Team WhatsApp button.
